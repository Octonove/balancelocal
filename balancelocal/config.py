"""Configuracion y rutas de datos de BalanceLocal (shim del nucleo compartido
...
LOG_PATH = get_data_dir() / "balancelocal.log"
DB_PATH = get_data_dir() / "actividad.db"


@dataclass
class AppConfig:
    output_dir: str = field(default_factory=lambda: str(default_output_dir()))
    registrar: bool = True            # rastrear actividad al abrir
    guardar_titulos: bool = True      # guardar el titulo de ventana (mas detalle, mas privado)
    inactividad_seg: int = 120        # sin teclado/raton = ausente
